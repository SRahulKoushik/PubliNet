...
$S_i^F\big(C_{pred},C_{ref}\big)=2*\frac{|F\big(U(C_{pred})_i)\cap F\big(U(C_{ref})_i\big)|}{|F\big(U(C_{pred})_i)\cup F\big(U(C_{ref})_i\big)|}$  
  
$S(p)=max_{C_{ref}\frac{1}{T}}\sum_{i=1}^T I[U(C_{pred})_i==U(C_{ref})_i]*S_i^F\big(C_{pred},C_{ref}\big)$

The overall score is then computed as follows:

Score(A(nlc)) = { m axp∈A(nlc)S(p), if∃p ∈ A(nlc)suchthatS(p) > 0 avgp∈A(nlc)S(p), otherwise
